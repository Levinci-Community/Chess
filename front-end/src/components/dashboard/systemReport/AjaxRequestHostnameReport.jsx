...
export default function AjaxRequestHostnameReport({ theme }) {
  ...
  useEffect(() => {
    axios
      .get(`${appSettings.API_PROXY}/dashboard/ajax_request_hostname`)
      .then((resp) => {
  ...
  const renderBarChart = (dataset, title) => {
    ...
    const chartData = [
      [t("dashboard.hostname"), t("dashboard.time")],
  ...
  return (
    ...
    >
      ...
      >
        ...
        {t("dashboard.ajax_request_hostname")}
      </Heading>
      <Flex>
        <Flex w={"50%"} h={"100%"}>
