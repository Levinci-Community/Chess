...
def generate_fens(engine, variant, min_depth, max_depth, add_move, required_pieces):
    ...
    start_fen = pf.start_fen(variant)
    ...
    while True:
        engine.newgame()
        move_stack = []
        while (pf.legal_moves(variant, start_fen, move_stack)
               and not pf.is_optional_game_end(variant, start_fen, move_stack)[0]):
            ...
            if not add_move:
                fen = pf.get_fen(variant, start_fen, move_stack)
                bestmove = None
            else:
                fen = pf.get_fen(variant, start_fen, move_stack[:-1])
...
if __name__ == '__main__':
    ...
    pf.set_option("VariantPath", engine.options.get("VariantPath", ""))
