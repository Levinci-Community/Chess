"""
    This module implements the pikafish class.
...
class PikafishException(Exception):
    pass


class Pikafish:
    """Integrates the pikafish chess engine with Python."""

    _del_counter = 0
    # Used in test_models: will count how many times the del function is called.

    def __init__(
        self, path: str = "Pikafish", depth: int = 15, parameters: dict = None
    ) -> None:
        self._DEFAULT_pikafish_PARAMS = {
            "Debug Log File": "",
            "Contempt": 0,
            "Min Split Depth": 0,
            "Threads": 1,
        ...
        self._path = path
        self._pikafish = subprocess.Popen(
            self._path,
        ...
        self._pikafish_major_version = self._read_line().split(" ")[1].split(".")[0].replace("-", "")
        ...
        self.update_engine_parameters(self._DEFAULT_pikafish_PARAMS)
    ...
    def get_parameters(self) -> dict:
        ...
        Returns:
            Dictionary of current pikafish engine's parameters.
        """
        return self._parameters

    def update_engine_parameters(self, new_param_valuesP: Optional[dict]) -> None:
        """Updates the pikafish parameters.
    ...
    def reset_engine_parameters(self) -> None:
        """Resets the pikafish parameters.
    ...
    def _put(self, command: str) -> None:
        if not self._pikafish.stdin:
            raise BrokenPipeError()
        if self._pikafish.poll() is None and not self._has_quit_command_been_sent:
            self._pikafish.stdin.write(f"{command}\n")
            self._pikafish.stdin.flush()
            if command == "quit":
                self._has_quit_command_been_sent = True

    def _read_line(self) -> str:
        if not self._pikafish.stdout:
            raise BrokenPipeError()
        if self._pikafish.poll() is not None:
            raise PikafishException("The pikafish process has crashed")
        return self._pikafish.stdout.readline().strip()
    ...
    def set_fen_position(
        ...
        Args:
            ...
            send_ucinewgame_token:
              Whether to send the "ucinewgame" token to the pikafish engine.
              The most prominent effect this will have is clearing Pikafish's transposition table,
    ...
    def set_position(self, moves: Optional[List[str]] = None) -> None:
        ...
        self.set_fen_position(
            "rnbakabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR  w - - 0 1", True
        )
    ...
    def get_board_visual(self, perspective_white: bool = True) -> str:
        ...
        while "Checkers" not in self._read_line():
            # Gets rid of the remaining lines in _pikafish.stdout.
            # "Checkers" is in the last line outputted by pikafish for the "d" command.
            pass
    ...
    def set_skill_level(self, skill_level: int = 20) -> None:
        """Sets current skill level of pikafish engine.
    ...
    def set_elo_rating(self, elo_rating: int = 1350) -> None:
        """Sets current elo rating of pikafish engine, ignoring skill level.
    ...
    def get_best_move_time(self, time: int = 1000) -> Optional[str]:
        ...
        Args:
            time:
              Time for pikafish to determine best move in milliseconds (int)
    ...
    def is_fen_valid(self, fen: str) -> bool:
        if not Pikafish._is_fen_syntax_valid(fen):
            return False
        temp_sf = Pikafish(path=self._path, parameters={"Hash": 1})
        ...
        except PikafishException:
            # If a PikafishException is thrown, then it happened in read_line() since the SF process crashed.
    ...
    def get_wdl_stats(self) -> Optional[List]:
        """Returns Pikafish's win/draw/loss stats for the side to move.
        ...
        if not self.does_current_engine_version_have_wdl_option():
            raise RuntimeError(
                "Your version of pikafish isn't recent enough to have the UCI_ShowWDL option."
            )
        self._go()
        lines = []
        while True:
    ...
    def does_current_engine_version_have_wdl_option(self) -> bool:
        """Returns whether the user's version of pikafish has the option
    ...
    def get_evaluation(self) -> dict:
        ...
        # pikafish shows advantage relative to current player. This function will instead
    ...
    def set_depth(self, depth_value: int = 2) -> None:
        """Sets current depth of pikafish engine.
    ...
    def get_what_is_on_square(self, square: str) -> Optional[Piece]:
        ...
        if piece_as_char == " ":
            ...
            return Pikafish.Piece(piece_as_char)
    ...
    def will_move_be_a_capture(self, move_value: str) -> Capture:
        ...
        Args:
            move_value:
                The proposed move, in the notation that pikafish uses.
                E.g., "e2e4", "g1f3", etc.
        ...
        if ending_square_piece != None:
            if self._parameters["UCI_Chess960"] == "false":
                return Pikafish.Capture.DIRECT_CAPTURE
            else:
                # Check for Chess960 castling:
                castling_pieces = [
                    [Pikafish.Piece.WHITE_KING, Pikafish.Piece.WHITE_ROOK],
                    [Pikafish.Piece.BLACK_KING, Pikafish.Piece.BLACK_ROOK],
                ]
                if [starting_square_piece, ending_square_piece] in castling_pieces:
                    return Pikafish.Capture.NO_CAPTURE
                else:
                    return Pikafish.Capture.DIRECT_CAPTURE
        elif move_value[2:4] == self.get_fen_position().split()[
            3
        ] and starting_square_piece in [
            Pikafish.Piece.WHITE_PAWN,
            Pikafish.Piece.BLACK_PAWN,
        ]:
            return Pikafish.Capture.EN_PASSANT
        else:
            return Pikafish.Capture.NO_CAPTURE

    def get_pikafish_major_version(self):
        """Returns pikafish engine major version.

        Returns:
            Current pikafish major version
        """

        return self._pikafish_major_version

    def is_development_build_of_engine(self) -> bool:
        """Returns whether the version of pikafish being used is a
           development build.
    ...
    def __del__(self) -> None:
        Pikafish._del_counter += 1
        if self._pikafish.poll() is None:
            self._put("quit")
            while self._pikafish.poll() is None:
                pass
